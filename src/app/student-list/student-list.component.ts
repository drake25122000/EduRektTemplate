...
export class StudentListComponent implements OnInit {
  ...
  tableColumns: string[] = ['matricId', 'name', 'year', 'course'];
    ...
    if (!this.formDialogOpened) {
      ...
      dialogRef.afterClosed().subscribe((result) => {
        ...
        if (result && result.delete) {
          this.confirmDelete(student.code);
        }
        this.reloadData();
      });
    }
  ...
  confirmDelete(code: string) {
    const dialogRef = this.dialog.open(ConfirmationDialogComponent, { data: `Student ${code}` });
    dialogRef.afterClosed().subscribe(
      (result) => {
        if (result.event === 'yes') {
          this.studentService.deleteStudent(code).subscribe(() => this.reloadData());
        }
      }
    );
  }
